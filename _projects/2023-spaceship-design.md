...
When feedback is added, the disturbance path becomes Gd(s)/(1 + C(s)Gu(s)) TL(s). As this loop gain increases, the disturbance sensitivity decreases, indicating that the controller should restore the speed more quickly and with smaller steady-state deviation. 
